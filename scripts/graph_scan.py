...
out = Path('.cache/graph'); out.mkdir(exist_ok=True)
print(f'{len(blocks)} blocks', flush=True)
for b in sorted(blocks, key=lambda b: b['dist_km']):
    path = out / f"{b['x']}_{b['y']}.json"
    if path.exists():
        continue
    t0 = time.time()
    try:
        g = build(b['x'], b['y'], b['x'] + B, b['y'] + B, f"{b['x']//1000}_{b['y']//1000}_")
    except Exception as exc:
        print(f"block {b['x']}_{b['y']} failed: {exc}", flush=True); continue
    path.write_text(json.dumps(g))
    print(f"block {b['x']}_{b['y']}: {len(g['parkings'])} parkings, {len(g['summits'])} summits, {len(g['runs'])} runs, {len(g['edges'])} edges, {time.time()-t0:.0f}s", flush=True)
