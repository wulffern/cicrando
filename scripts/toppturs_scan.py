...
out = Path('.cache/toppturs'); out.mkdir(exist_ok=True)
print(f'{len(blocks)} blocks within {radius} km', flush=True)
for b in blocks:
    ...
    t0 = time.time()
    try:
        r = toppturs(b['x'], b['y'], b['x'] + B, b['y'] + B)
    except Exception as exc:
        print(f"block {b['x']}_{b['y']} failed: {exc}", flush=True); continue
    path.write_text(json.dumps(r))
    print(f"block {b['x']}_{b['y']}: {len(r['tours'])} summits, {sum(1 for t in r['tours'] if t['descent'])} with descent, {time.time()-t0:.0f}s", flush=True)
